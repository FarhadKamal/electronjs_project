  ...
  get_by_id(id, function (row) {
    ...
    get_raw_material_list_by_id(id, function(rows_raw_material){
      ...
      rows_raw_material.forEach(function (raw_material){
        ...
        raw_material_table += "<input type='hidden' name='raw_materials_id[]' id='raw_materials_id' value = '" + raw_material.raw_mat_id + "' />";
        raw_material_table += "</td>";
        raw_material_table += "<td>";
        raw_material_table += "<input type='number' name='raw_materials_qty[]' id='raw_materials_qty' value='" + raw_material.qty + "' class='form-control' min='0' step='Any' />";
        raw_material_table += "</td>";
        raw_material_table += "</tr>";
...
ipcMain.on("semi:good:add:loaded", function (e, id) {
  ...
  get_raw_material_list(function(rows){
    ...
    rows.forEach(function (row){
      ...
      html += "<input type='hidden' name='raw_materials_id[]' id='raw_materials_id' value = '" + row.raw_mat_id + "' />";
      html += "</td>";
      html += "<td>";
      html += "<input type='number' name='raw_materials_qty[]' id='raw_materials_qty' value='0' class='form-control' min='0' step='Any' />";
      html += "</td>";
      html += "</tr>";
...
ipcMain.on("semi:good:edit:loaded", function (e, id) {
  ...
  e.reply("semi:good:edit:fetched", [trackData, trackUnitData, raw_material_table]);  
...
>>>>>>> Stashed changes
  ...
  connection.query(query, function (err, result) {
    ...
    if (err) {
      ...
      const raw_material_id = item[2];
      const raw_material_qty = item[3];
      let insert_query = '';
      ...
      query_semi_good_component = "INSERT INTO semi_good_components (semi_good_id, raw_mat_id, qty) VALUES "+test_query;
...
function update(item, callback) {
  ...
  connection.query(query, function (err, result) {
    ...
    delete_semi_goods_componenet(item);

    // Inserting New Components After Deleting The Previous One
    insert_semi_goods_component_after_update(item);
...
function get_raw_material_list(callback){
  ...
  // Perform a query
  query = "select raw_material_list.raw_mat_id, CONCAT(raw_material_list.raw_mat_name,' (',raw_material_list.raw_mat_unit,')') AS raw_mat_name, 0 AS qty From raw_material_list";
  // console.log(query);
  connection.query(query, function (err, rows, fields) {
    if (err) {
      console.log("An error ocurred performing the query.");
...
function get_raw_material_list_by_id(id, callback) {
  ...
  // Perform a query
  query = "SELECT semi_good_components.raw_mat_id, CONCAT(raw_material_list.raw_mat_name,' (',raw_material_list.raw_mat_unit,')') AS raw_mat_name, \
           semi_good_components.qty \
            ...
            WHERE semi_good_components.semi_good_id = "+id+" AND semi_good_components.qty > 0 \
            UNION \
            ...
            (SELECT raw_mat_id from semi_good_components WHERE semi_good_components.semi_good_id = "+id+" AND semi_good_components.qty > 0)";
...
function delete_semi_goods_componenet(item){
  ...
  query = "delete from semi_good_components where semi_good_id=" +item[0];
  console.log(query);
  connection.query(query, function (err, result) {
    if (err) {
      console.log("An error ocurred performing the query.");
...
function insert_semi_goods_component_after_update(item){
  ...
  const raw_material_id = item[3];
  const raw_material_qty = item[4];
  let insert_query = '';


  for (let index = 0; index < raw_material_id.length; index++) {
    const semi_item_id = item[0];
    const raw_mat_id = raw_material_id[index];
    const qty = raw_material_qty[index];
    insert_query += '(' +semi_item_id+','+raw_mat_id+','+qty+'),';
  }
  ...
  let test_query = insert_query.slice(0, -1);
  query = "insert into semi_good_components (semi_good_id, raw_mat_id, qty) VALUES "+test_query;
